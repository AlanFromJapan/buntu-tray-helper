...
def background_task():
    ...
    while not __thread_kill:
        ...
        for device in config.get('devices', []):
            ...
            if mac_address != '':
                try:
                    print(f"Preparing to send WOL packet to '{device.get('name', '<unknown>')}' [{mac_address}] ...")
                    ...
                    print(f"WOL packet sent to '{device.get('name', '<unknown>')}' [{mac_address}].")
                except Exception as e:
                    print(f"Error sending WOL packet to {mac_address}: {e}")
